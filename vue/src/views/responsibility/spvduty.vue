<template>
  <div class="app-container" >
    ...
    <div class="filter-container" align="center"  style="margin-top: 20px;">
      <!-- v-waves -->
      <label>责任监督标题</label>&nbsp;&nbsp;
      <el-input v-model="dutyTitle" placeholder="请输入责任监督标题" style="width: 200px;" class="filter-item"/>
      &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <el-table-column type="selection" width="60px" align="center"></el-table-column>
      <el-table-column label="序号" prop="index"  align="center" width="100px" type="index" :index="indexMethod">
      ...
      <el-table-column label="责任监督编号" prop="dnumId"   align="center" width="170px">
        ...
        <template slot-scope="scope">
          <span  >{{ scope.row.status===0?'结束':(scope.row.status===1?'通知':(scope.row.status===2?'自查':(scope.row.status===3?'在检查':(scope.row.status===4?'整改通知':'通报'))))}}</span>
        </template>
        </el-table-column>
      <el-table-column label="操作"    align="center" width="225px">
        <template slot-scope="scope">
           <a style="color:#1890ff" @click="handleUpdate(scope.row)">{{ scope.row.status===1||scope.row.status===4||scope.row.status===6?'编辑':''}}&nbsp;&nbsp;&nbsp;</a>
           <a style="color:#1890ff" @click="handleUpdate(scope.row)">{{ scope.row.status===2||scope.row.status===3?'查看反馈':(scope.row.status===1?'发布通知':(scope.row.status===4?'发布整改通知':(scope.row.status===6?'发布通报':'')))}}</a>
        </template>
      </el-table-column>
    </el-table>
...
        </quill-editor>
    ...
    :format="'yyyy-MM-dd HH:mm:ss'">
        ...
        <el-form-item style="font-weight: bold;" label="创建人" prop="sysStaff" >
          <el-input v-model="temp.sysStaff.name" placeholder="请输入创建人" style="width:100%" disabled="disabled" />
        </el-form-item>
                </el-col>
        ...
        <el-row>
                <el-col style="width:43%">
        <el-form-item style="font-weight: bold;" label="附件相关" prop="dutyTitle" >
  ...
  <div slot="tip"  class="el-upload__tip">只能上传单个txt/word/pdf文件，且不超过500k</div>
        ...
        <el-form-item style="font-weight: bold;" label="状态" prop="dstatus" >
          <el-input v-model="temp.dstatus" placeholder="请输入制度信息标题" style="width:100%" disabled="disabled"/>
        </el-form-item>
                </el-col>
        </el-row>
      </el-form>
